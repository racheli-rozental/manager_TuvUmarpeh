...
const UserRegistration = () => {
    ...
    const handleSubmit = async (e: any) => {
        ...
        try {
            ...
            setAlert(true); // הצג את ההודעה
            setTimeout(() => setAlert(false), 3000); // הסתר אחרי 3 שניות
        } catch (error: any) {
    ...
    const downloadFile = async (fileName: string) => {
        try {
            const response = await fetch(`"https://server-angular-tovumarpeh.onrender.com"/files/${fileName}`);
